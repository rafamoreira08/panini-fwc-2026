  ...
  ({ className, variant = 'primary', size = 'md', ...props }, ref) => {
    return (
      <button
        ref={ref}
        className={cn(
          'inline-flex items-center justify-center font-medium rounded-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer touch-manipulation',
          {
          ...
          {
            'px-3 py-1.5 text-xs font-medium min-h-[36px]': size === 'sm',
            'px-4 py-2.5 text-sm font-medium min-h-[44px]': size === 'md',
            'px-6 py-3 text-base font-semibold min-h-[48px]': size === 'lg',
          },
          className
        )}
        {...props}
